...
export class ThemeService {
  ...
  async initializeJekyllTheme(
    ...
    try {
      await fs.promises.mkdir(path.join(repoPath, '_layouts'), { recursive: true });
      await fs.promises.mkdir(path.join(repoPath, '_includes'), { recursive: true });
      await fs.promises.mkdir(path.join(repoPath, 'assets', 'css'), { recursive: true });
      await fs.promises.mkdir(path.join(repoPath, 'assets', 'js'), { recursive: true });
      await fs.promises.mkdir(path.join(repoPath, '.github', 'workflows'), { recursive: true });
      ...
      await fs.promises.writeFile(path.join(repoPath, '_config.yml'), configContent, 'utf8');

      // Write assets/css/style.css
      await fs.promises.writeFile(path.join(repoPath, 'assets', 'css', 'style.css'), cssTemplate, 'utf8');

      // Write assets/js/site-data.js (Processed by Jekyll to generate JSON array of pages)
      await fs.promises.writeFile(path.join(repoPath, 'assets', 'js', 'site-data.js'), siteDataTemplate, 'utf8');

      // Write assets/js/app.js (TOC Builder, ScrollSpy, File Tree with search)
      await fs.promises.writeFile(path.join(repoPath, 'assets', 'js', 'app.js'), appTemplate, 'utf8');

      // Write _layouts/default.html
      await fs.promises.writeFile(path.join(repoPath, '_layouts', 'default.html'), defaultLayoutTemplate, 'utf8');
...
`;
      if (!fs.existsSync(path.join(repoPath, 'index.md'))) {
        await fs.promises.writeFile(path.join(repoPath, 'index.md'), indexContent, 'utf8');
      }

      new Notice("Jekyll templates and GitHub workflow successfully initialized!");
    } catch (err) {
      new Notice("Template Init Error: " + err.message);
    }
  }
